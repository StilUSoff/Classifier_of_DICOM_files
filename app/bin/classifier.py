...
#     test_dataset = MedicalDataset('app/bin/classifier/val.csv', attributes, val_transform)
...
import csv
import sys
sys.path.append(os.path.join(electron_root, 'app', 'bin', 'classifier'))
from dataset import MedicalDataset, AttributesDataset, mean, std
from model import MultiOutputModel
from model_test import checkpoint_load
from jpg_rgb_refactor import converting
...
device = torch.device("cuda" if torch.cuda.is_available() and device == 'cuda' else "cpu")
# Установка рабочей директории в корень проекта Electron
electron_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
os.chdir(electron_root)
