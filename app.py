####### ПОПЫТКА ДОБИТЬ ПРОГРЕСБАР
####### отображение ошибок в случае сбоев работы функций и модулей
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.combobox = ctk.CTkComboBox(self.main_frame,border_color = self.main_color, button_color = self.main_color, button_hover_color=self.hover_color,width=200, fg_color=self.main_color, justify="center", values=["Classifire of images", "Sort DICOM files in path", "DICOM refactor to jpg", "Split data for model", "Train model"], command=self.combobox_callback)
    ...
    def prepare_train(self):
        ...
        start = ctk.CTkButton(self.center_frame, hover_color=self.hover_color,fg_color=self.main_color,text="Start", command=self.run_pr_tr)
        start.pack(pady=20)
        if torch.cuda.is_available():
            self.current_buttons.extend([checkbox])
        self.current_buttons.extend([folder_button, file_button, save_checkpoints, self.N_epochs, self.batch_size, self.num_workers, start, textbox_csv, textbox_atr])
    ...
    def run_classifier(self, folder_path, check):
        self.current_buttons[3].configure(fg_color=self.chosen_color)
        self.progress_bar = ctk.CTkProgressBar(self.center_frame, mode="indeterminate")
        ...
        self.current_buttons[0].configure(fg_color=self.main_color) 
    ...
    def run_sort(self, folder_path, body):
        ...
        if not_sorted is None:
            ...
            string = ''
            textbox = ctk.CTkTextbox(master=self.center_frame, width=700, height=400, corner_radius=2)
    ...
    def run_split_data(self, folder_path, work_path, atribut):
        ...
        if hasattr(self, 'progress_bar') and self.progress_bar:
            self.progress_bar.destroy()
        self.current_buttons[0:3].configure(fg_color=self.main_color) 

    def run_train(self):
        self.current_buttons[6].configure(fg_color=self.chosen_color)
        folder_path = self.tr_folder_path
        atr_folder_path = self.tr_atr_folder_path
        check_saves = self.check_saves_ans
        gpu_choice = self.gpu_choice
        self.bar_tr = ctk.CTkProgressBar(master=self.center_frame, fg_color=self.main_color, determinate_speed=(1/float(self.N_epochs_wr)))
        ...
        train_object.train_scipt(N_epochs=int(self.N_epochs_wr), batch_size=int(self.batch_size_wr), num_workers=int(self.num_workers_wr))
        messagebox.showinfo(title="Done!", message="You're all set")
        if hasattr(self, 'bar_tr') and self.bar_tr:
            self.bar_tr.destroy()
        self.current_buttons[0:2].configure(fg_color=self.main_color)
