...
def visualize_grid(model, data_loader, attributes, device, checkpoint=None):
    ...
    df = pd.read_csv(resource_path('app\\.work_labels.csv'))
...
def create_csv(directory=None,modalities=None,bodyparts=None,names=None,check=None):
    ...
    with open(resource_path('app\\.work_labels.csv'), 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(data)
    if check==1:
        os.remove(resource_path('app\\.work_labels.csv'))
        return data
...
def main(work_folder, save=None, val=None):
    if save is None and val is None:
        ...
        attributes = AttributesDataset(resource_path('app\\bin/classifier/val.csv'))
    else:
        checkpoint = save
    ...
    test_dataset = MedicalDataset(resource_path('app\\.work_labels.csv'), attributes, val_transform)
